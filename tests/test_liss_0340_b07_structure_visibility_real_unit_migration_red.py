"""AT-TDD: LISS-0340 -- migrate B07_structure_visibility to real units
(WP-0095 work unit 8).

Design decision: docs/issues/LISS-0340-b07-structure-visibility-real-unit-migration.md
"""
...
_B07_PATH = str(
    _REPO / "examples/basics/B07_structure_visibility/structure_visibility.sqx"
)
...
def test_b07_compiles_and_runs_to_a_real_terminal_measurement() -> None:
    compiled = compile_path(_B07_PATH)
    hard_compile = _hard(compiled.diagnostics)
    assert compiled.ok and not hard_compile, hard_compile

    result = run_path(
        _B07_PATH, settings={"target": "local", "seed": 0}, stdout=io.StringIO()
    )
