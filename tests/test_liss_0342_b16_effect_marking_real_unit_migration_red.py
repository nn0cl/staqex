"""AT-TDD: LISS-0342 -- migrate B16_effect_marking to real units
(WP-0095 work unit 10).

Design decision: docs/issues/LISS-0342-b16-effect-marking-real-unit-migration.md
"""
...
_B16_PATH = str(
    _REPO / "examples/basics/B16_effect_marking/effect_marking.sqx"
)
...
def test_b16_compiles_and_runs_to_a_real_terminal_measurement() -> None:
    compiled = compile_path(_B16_PATH)
    hard_compile = _hard(compiled.diagnostics)
    assert compiled.ok and not hard_compile, hard_compile

    result = run_path(
        _B16_PATH, settings={"target": "local", "seed": 0}, stdout=io.StringIO()
    )
