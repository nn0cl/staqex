"""AT-TDD: LISS-0333 -- migrate A05_qaoa_portfolio to dimensioned
(arbitrary-cost-unit) Hamiltonians (WP-0095 work unit 3).

Design decision: docs/issues/LISS-0333-a05-qaoa-arbitrary-unit-migration.md
"""
...
_A05_PATH = str(
    _REPO / "examples/applied/A05_qaoa_portfolio/main_qaoa_portfolio.sqx"
)
...
def test_a05_compiles_and_runs_to_a_real_terminal_measurement() -> None:
    compiled = compile_path(_A05_PATH)
    hard_compile = _hard(compiled.diagnostics)
    assert compiled.ok and not hard_compile, hard_compile

    result = run_path(
        _A05_PATH, settings={"target": "local", "seed": 0}, stdout=io.StringIO()
    )
