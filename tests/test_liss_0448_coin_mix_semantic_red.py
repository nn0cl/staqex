...
def test_liss_0448_mix_preserves_control_and_branch_rules() -> None:
    ...
    assert mixture.branch_rules == (
        (
            ("pattern", 0),
            ("is_else", False),
            ("source_node_id", arm_nodes[0].node_id),
        ),
        (
            ("pattern", None),
            ("is_else", True),
            ("source_node_id", arm_nodes[1].node_id),
        ),
    )
...
def test_liss_0448_branch_changes_update_semantic_fingerprint() -> None:
    original = compile_source(FIXTURE.read_text(encoding="utf-8"))
    mutated = compile_source(
        FIXTURE.read_text(encoding="utf-8").replace(
            "0 -> Dirac(0)", "1 -> Dirac(0)", 1
        )
    )
    assert original.ok, original.diagnostics
    assert mutated.ok, mutated.diagnostics
    assert original.scientific_semantic_ir is not None
    assert mutated.scientific_semantic_ir is not None
    assert semantic_fingerprint(original.scientific_semantic_ir) != semantic_fingerprint(
        mutated.scientific_semantic_ir
    )
