# WP-0143: 連続場・tensor・偏微分・IC/BCのsource契約
...
| Issue | [LISS-0526](../issues/LISS-0526-continuous-field-semantic-contract.md) |
| Depends on | [WP-0133](WP-0133-scientific-typed-bindings.md) |
| Blocks | WP-0144, WP-0145, WP-0146, WP-0154 |
...
| Acceptance | [Scientific Workflow specification](../specs/staqex-scientific-workflow-acceptance.md), F01 |
| Implementation permission | no; no Phase 1 approval |
| Current Next Issue | LISS-0526 Phase 0 acceptance/profile review only |

## Scope

一つのscalar/vector/tensor PDE profileでsourceから構造化IRへfield/domain/partial derivative/IC/BCを保持する。

## Out of scope

全方程式parser、数値Solver、新独立IR、既存product/tensor完了sliceの再開。 共通除外: branch/commit/PR操作は今回禁止、provider SDK/認証/live QPUは独立WP。

## Acceptance scenarios and verification

F01のGiven/When/Thenを適用する。具体的検証: parser→HIR→IR→inspectionを確認。unit/basis/axis/BC不一致と正当な隣接式を対にする。
...
opaque式文字列やcaller DTOをsource対応と誤認。既存式syntaxとの衝突。
...
0. このWPのscopeとF01の具体fixture/期待値をreview。ADR承認とreadinessを確認。
...
- ID: AIP-WP-0143-2026-09-08-001; status: proposed.
...
- **Unit A / F01-a / M**: scalar fieldのdomain/partial derivative/IC/BCを一つのsource profileからcanonical IRへ接続。数値計算を除外。
- **Unit B / F01-b / M**: Aをvector/tensor axis/basisへ拡張し、縮約/微分の型・次元と正当/不正sourceを検証。新Solverを除外。
