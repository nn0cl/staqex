...
| Field | Value |
|---|---|
| Status | **open — bounded observation and scientific-lexicon metadata contracts shipped via LISS-0481/LISS-0480; public observation execution and remaining design/implementation slices remain open** (2026-09-04) |
...
| Implementation | `DiagnosticView<T>` classification shipped (PR #342); bounded observation metadata and collapse/lane contract shipped via [LISS-0481](../issues/LISS-0481-observation-contract.md); bounded scientific lexicon metadata shipped via [LISS-0480](../issues/LISS-0480-scientific-lexicon-contract.md); `mix` canonical grammar and `when` hard-retirement diagnostic shipped (PR #337, commit `321de3a`, under ADR 0190/WP-0093 Phase 2 approval); `superpose` formal grammar/AST/type boundary **complete** on [LISS-0320](../issues/LISS-0320-superpose-formal-grammar.md), PR #345 — see below; `controlled(...)` call-form execution already shipped (confirmed live 2026-08-05, see work unit 2's correction note) — no separate Issue needed for it; remaining public observation execution, further scientific lexicon, and conformance changes still require their own approval |
...
`Observation<T>`), and remaining conformance scenarios are still review
questions. The bounded observation semantic mapping contract is recorded as
complete in [LISS-0482](../issues/LISS-0482-observation-semantic-mapping.md);
it does not authorize public observation execution.
