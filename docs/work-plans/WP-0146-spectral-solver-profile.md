...
| Issue | [LISS-0529](../issues/LISS-0529-spectral-solver-profile.md) |
...
| Acceptance | [Scientific Workflow specification](../specs/staqex-scientific-workflow-acceptance.md), N03 |
| Implementation permission | no; no Phase 1 approval |
| Current Next Issue | LISS-0529 Phase 0 acceptance/profile review only |

## Scope

periodic線形場のbasis/mode/normalization/dealias/BC契約を一つ接続する。

## Out of scope

全basis、非周期問題の自動近似、AMR。 共通除外: branch/commit/PR操作は今回禁止、provider SDK/認証/live QPUは独立WP。

## Acceptance scenarios and verification

N03のGiven/When/Thenを適用する。具体的検証: 既知modeとcutoff収束。非周期BC、aliasing policy欠落を拒否。
...
basis normalizationと物理unitの混同、aliasing隠蔽。
...
0. このWPのscopeとN03の具体fixture/期待値をreview。ADR承認とreadinessを確認。
...
- ID: AIP-WP-0146-2026-09-08-001; status: proposed.
