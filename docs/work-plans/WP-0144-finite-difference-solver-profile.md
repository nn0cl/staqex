# WP-0144: 有限差分RealizeとSolverPort
...
| Issue | [LISS-0527](../issues/LISS-0527-finite-difference-solver-profile.md) |
| Depends on | [WP-0143](WP-0143-continuous-field-semantic-contract.md) |
| Blocks | WP-0147, WP-0155 |
...
| Acceptance | [Scientific Workflow specification](../specs/staqex-scientific-workflow-acceptance.md), N01 |
| Implementation permission | no; no Phase 1 approval |
| Current Next Issue | LISS-0527 Phase 0 acceptance/profile review only |

## Scope

熱方程式の一つのFDM profileでgrid/stencil/time-step/BC/error policyとSolverResultを接続する。

## Out of scope

FEM/スペクトル/AMR、数値libraryの再実装、黙示step選択。 共通除外: branch/commit/PR操作は今回禁止、provider SDK/認証/live QPUは独立WP。

## Acceptance scenarios and verification

N01のGiven/When/Thenを適用する。具体的検証: manufactured解とgrid refinementで収束次数区間・BC・誤差を検査。不安定stepは明示診断。
...
安定性条件だけを精度保証と誤認。empirical errorのbound表記。
...
0. このWPのscopeとN01の具体fixture/期待値をreview。ADR承認とreadinessを確認。
...
- ID: AIP-WP-0144-2026-09-08-001; status: proposed.
