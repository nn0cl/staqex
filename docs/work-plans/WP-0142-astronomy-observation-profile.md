# WP-0142: ObsCore/VOTable天文観測縦切り
...
| Current Next Issue | LISS-0525 Phase 0 acceptance/profile review only |

## Scope

一つのflux/time-series profileでDataset発見metadata、table、calibration、uncertainty、time/frameを保持する。

## Out of scope

天文table/coordinate parserの再実装、全survey統合、観測所操作。 共通除外: branch/commit/PR操作は今回禁止、provider SDK/認証/live QPUは独立WP。

## Acceptance scenarios and verification

A01のGiven/When/Thenを適用する。具体的検証: 既知signal、upper limit/null、time scale/frame不一致、calibration版、coverage不足の成功/拒否対。
...
0. このWPのscopeとA01の具体fixture/期待値をreview。ADR承認とreadinessを確認。
