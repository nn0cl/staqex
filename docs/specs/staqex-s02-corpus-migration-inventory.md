...
|---|---|---|---|---|---|---|
| B04 | `U_dur = exp(-i*H*dur/hbar)`; `Evolve() { U_dur * psi }` at `examples/basics/B04_evolve_not_loops/evolve_not_loops.sqx:1-15` | Classical=`dur`; Mathematical=`exp` operand structure; Quantum=`H`, `State`; Finite=`N/A`; Host=`N/A` | SV-09 B04; SV-13 evolve checks | exact local propagator; no finite claim | `evolve times`/state boundary; unsupported target behavior follows existing QPU diagnostics | N/A; teaching fixture has no numerical baseline |
| B08 | Named `H_chain`, exact `U_dur`, state application at `examples/basics/B08_operators_hamiltonians/operators_hamiltonians.sqx:1-20` | Classical=constants/`dur`; Mathematical=operator algebra; Quantum=`H_chain`, `State`; Finite=`N/A`; Host=`N/A` | SV-09 B08; SV-19 operator/exponential checks | exact simulator lane | non-unitary/unsupported target lowering uses existing diagnostics; no hidden finiteization | N/A; no migration baseline |
| B10 | Static target operation and terminal observation at `examples/basics/B10_static_qpu_lane/main_static_qpu_lane.sqx:1-9` | Classical=target parameters; Mathematical=`N/A`; Quantum=observed state; Finite=static gate/QASM lane; Host=resource/target selection | SV-09 B10; SV-10 target-QPU emission | provider-neutral finite static lane; no live submission | target capability/QASM rejection remains explicit | N/A; no numerical migration |
| B12 | Density/open-system source at `examples/basics/B12_open_systems/main_open_systems.sqx:1-25` | Classical=coefficients; Mathematical=Lindblad expression; Quantum=density state; Finite=RK4 simulator step; Host=`N/A` | SV-09 B12; applicable SV open-system checks | declared simulator contract, not QPU claim | adaptive/QPU execution intentionally unsupported by scope | N/A; no numerical migration |
| B13 | Host `Job`/measurement boundary at `examples/basics/B13_host_job_api/main_host_job.sqx:1-6` | Classical=job metadata/shots; Mathematical=`N/A`; Quantum=measured state; Finite=`N/A`; Host=`Job`/result orchestration | SV-09 B13 | local/provider-neutral Host DTO boundary | provider submission remains outside this inventory | N/A; API fixture |
| B14 | Resource profile and terminal measurement at `examples/basics/B14_resource_profile/main_resource_profile.sqx:1-6` | Classical=`N/A`; Mathematical=`N/A`; Quantum=state/measure; Finite=local resource estimate; Host=resource budget/profile | SV-09 B14; resource checks | local resource enforcement; no provider price/SDK | budget/capability rejection remains fail-closed | N/A; resource fixture |
| A03 | H₂-style `H`, `U_t = exp(-i*H*dur/hbar)`, tuple state at `examples/applied/A03_h2_vqe/main_h2_vqe.sqx:1-46` | Classical=molecular coefficients; Mathematical=Hamiltonian terms; Quantum=tuple `State`; Finite=`N/A`; Host=`N/A` | SV-09 A03; SV-19 arbitrary-H checks | exact local evolution; no chemistry/QPU claim | unsupported target lowering remains explicit through existing capability checks | N/A; no locked baseline |
| A05 | Named mixer/cost exponentials and repeated `Evolve` at `examples/applied/A05_qaoa_portfolio/main_qaoa_portfolio.sqx:1-38` | Classical=layer durations/parameters; Mathematical=cost/mixer expressions; Quantum=propagators/State; Finite=`N/A`; Host=`N/A` | SV-09 A05 | exact local propagators; not optimizer or finite-QPU claim | target limits remain existing capability boundaries; no silent QAOA interpretation | N/A; no quality baseline |
| A07 | Open-system sensor and terminal observation at `examples/applied/A07_open_system_sensor/main_open_system_sensor.sqx:1-33` | Classical=sensor inputs; Mathematical=open-system terms; Quantum=density evolution; Finite=simulator realization; Host=local input orchestration | SV-09 A07 | simulator-only partial support | adaptive/QPU follow-up intentionally unsupported | N/A; no benchmark baseline |
| A11 | Named `U_t` and tuple-state evolution at `examples/applied/A11_noether_forge/main_static.sqx:1-73` | Classical=parameters; Mathematical=Noether/Hamiltonian expressions; Quantum=tuple State; Finite=`N/A`; Host=module/import orchestration only | SV-09 A11; SV-19 evolution; import/link checks | exact local lane; no live target claim | module/link and target capability diagnostics remain explicit | N/A; no migration baseline |
| S02 | `psi_0`, `P_F`, `psi_sel`, exact `U_t`, formal `Limit`, explicit `Realize` at `examples/showcase/S02_drug_discovery/main_selection.sqx:75-155` | Classical=Host arrays/baseline; Mathematical=`Sigma`/`Set`; Quantum=State/Operator; Finite=`Realize` plan; Host=runner/input/shots | direct S02 compile; SV-09 excludes S02; LISS-0438 5/5; Host run; baseline 25/256 | exact local `U_t`; finite `U_qpu` capability-rejected/not submitted | `QASM_TROTTER_UNSUPPORTED_H`; no partial artifacts/provenance | pre-migration hash/metrics preserved as reference; current hash separate |
